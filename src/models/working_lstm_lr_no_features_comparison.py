...
def to_supervised(train, n_input, n_out):
    ...
    for _ in range(len(train)):  # Loop over the dataset
        ...
        if out_end <= len(train):  # Ensure indices are within bounds
            X.append(train[in_start:in_end, -1].reshape((n_input, 1)))  # Append input sequence (reshape to 2D array)
    ...
    print(f'X shape: {X.shape}, y shape: {y.shape}')  # Print shapes of X and y
    return X, y  # Return input and output sequences as numpy arrays
...
def forecast_lstm(model, history, n_input):
    ...
    input_x = data[-n_input:, -1].reshape((n_input, 1))  # Get the last n_input sequences (target only)
...
def forecast_lr(model, history, n_input):
    data = np.array(history)  # Convert history to numpy array
    input_x = data[-n_input:, -1].reshape(1, -1)  # Get the last n_input sequences (target only) and flatten
...
output_folder = 'No_Features_Model_Visualizations'
n_input = 4  # Number of input sequences
n_out = 4  # Number of output sequences
